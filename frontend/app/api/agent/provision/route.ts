...
export async function POST(request: Request) {
  try {
    const response = await fetch(`${backendUrl}/api/agent/provision`, {
      method: "POST",
      headers: { "x-agent-api-key": process.env.AGENT_BACKEND_API_KEY ?? "", "x-wallet-token": sessionToken(request) },
      cache: "no-store",
    });
